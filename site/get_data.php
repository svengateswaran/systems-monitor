...
function print_data($title, $data_file, $suffix, $util, $rowspan) {
  if(file_exists($data_file)) {
    ...
    $util_style = "";
    if(!$util)
      $util_style = "style='background:#ddd'";

    if($suffix == "%") {
      if($data < 0 or $data > 100)
         $data = 0;
      echo "<td rowspan=$rowspan> <div class='progress-bar' style='width:" . (int)$data . "%'>" . $data . "%</div></td>";
    } else {
      echo "<td rowspan=$rowspan> $data $suffix </td>";
    }
    return 0;
 }
 return -1;
...
foreach ($dir_itr as $fileinfo) {
  if (!$fileinfo->isDot()) {
       ...
       for($i = 0; $i < count($data_col); $i++){
         ...
         if($data_col[$i][1] != "gpu_count") {
           print_data($data_col[$i][0], $data_file, $data_col[$i][2], 1, $gpu_count);
         } else {
